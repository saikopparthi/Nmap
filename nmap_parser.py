    ...
    def parse(self, output: str) -> Dict:
        ...
        for line in lines:
            if 'Nmap scan report for' in line:
                ...
                result['ip'] = parts[5].strip('()') if len(
                    parts) > 5 else parts[4]
            elif 'Host is up' in line:
                latency_match = re.search(r'\((.*?)\s+latency\)', line)
                result['latency'] = latency_match.group(
                    1) if latency_match else None
                ...
                script_output = line.split(
                    ':', 1)[1].strip() if ':' in line else line.split(
                        '|', 1)[1].strip()
